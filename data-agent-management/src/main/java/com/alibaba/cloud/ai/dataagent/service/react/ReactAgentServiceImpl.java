...
import com.alibaba.cloud.ai.dataagent.interceptor.LogToolInterceptor;
...
public class ReactAgentServiceImpl implements ReactAgentService {
  ...
  private final LogToolInterceptor logToolInterceptor;

  public ReactAgentServiceImpl(ExecutorService executor,
      AgentRegistry agentRegistry,
      MultiTurnContextManager multiTurnContextManager,
      LogToolInterceptor logToolInterceptor) throws GraphStateException {
    this.executor = executor;
    this.agentRegistry = agentRegistry;
    this.multiTurnContextManager = multiTurnContextManager;
    this.logToolInterceptor = logToolInterceptor;
    
    // 添加工具调用回调
    this.logToolInterceptor.addToolCallCallback(new LogToolInterceptor.ToolCallCallback() {
      @Override
  ...
  public void reactStreamProcess(Sinks.Many<ServerSentEvent<GraphNodeResponse>> sink,
      String agentId, String threadId, String query)
      throws com.alibaba.cloud.ai.graph.exception.GraphRunnerException {
    ReactAgent reactAgent = agentRegistry.getReactAgent();
    ...
    // 过滤掉开始和结束事件
    Flux<?> nodeOutputFlux = reactAgent.stream(query, config)
  ...
  private void handleNodeOutput(Object nodeOutput, String agentId, String threadId) {
    ...
    if (nodeOutput instanceof StreamingOutput streamingOutput) {
      ...
      try {
        log.info("----- Received non-StreamingOutput node output: {}", JsonUtil.getObjectMapper().writeValueAsString(nodeOutput));
      } catch (JsonProcessingException e) {
        throw new RuntimeException(e);
      }
      try {
        Map<String, Object> data = ((com.alibaba.cloud.ai.graph.NodeOutput) nodeOutput).state()
            .data();
        String text;
        if (data.containsKey("messages")) {
          text = String.valueOf(data.get("messages"));
        } else {
          // 检测工具调用信息
          if (data.containsKey("tool_calls")) {
            Object toolCalls = data.get("tool_calls");
            text = "工具调用: " + toolCalls.toString();
            log.info("Tool call detected: {}", toolCalls);
          } else if (data.containsKey("tool_call")) {
            Object toolCall = data.get("tool_call");
            text = "工具调用: " + toolCall.toString();
            log.info("Tool call detected: {}", toolCall);
          } else {
            text = nodeOutput.toString();
          }
        }
        ...
        GraphNodeResponse response = GraphNodeResponse.builder()
            .agentId(agentId)
            .threadId(threadId)
            .nodeName(((NodeOutput) nodeOutput).node())
            .text(text)
            .textType(TextType.TEXT)
            .build();
        context.getSink().tryEmitNext(ServerSentEvent.builder(response).build());
      } catch (Exception e) {
        log.error("Error processing node output: {}", e.getMessage(), e);
      }
    }
  }
  ...
  private void handleStreamNodeOutput(String threadId, String agentId, StreamingOutput output) {
    ...
    if(RunnableConfig.AGENT_TOOL_NAME.equals(node)) {
  ...
  private void handleToolCall(String threadId, String toolName, String input, String output) {
    log.info("handleToolCall: threadId={}, toolName={}, input={}, output={}", threadId, toolName, input, output);
    ...
    try {
      // 构建工具调用的输入和输出信息
      String toolCallInfo = "工具调用: " + toolName + "\n" +
          "输入: \n ```json\n" + input + "\n```\n" ;
