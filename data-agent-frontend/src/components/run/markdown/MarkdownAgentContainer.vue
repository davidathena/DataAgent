...
import MarkdownItEcharts from './markdown-plugin-echarts'
import MarkdownItHighlight from './markdown-plugin-highlight'
...
export default {
    ...
    this.renderECharts = debounce(() => {
      ...
      echartsElements.forEach(element => {
        try {
          ...
          // 再次验证JSON结构是否完整
          const hasValidJson = /\{[\s\S]*\}/.test(content) && content.match(/\{/g)?.length === content.match(/\}/g)?.length;
          if (hasValidJson) {
            ...
            console.log('ECharts configuration is incomplete, skipping rendering', element.textContent);
          }
        } catch (e) {
          // 只在控制台记录错误，不影响用户界面
  ...
  watch: {
    content: {
      ...
      handler(val) {
        this.$nextTick(() => {
          this.$refs['markdown-agent-container'].innerHTML = this.md.render(
            val
          );
          // 调用防抖后的ECharts渲染函数
          this.renderECharts();
